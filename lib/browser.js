    ...
    Object.keys(nameMap).forEach(function (name) {
      ...
      matches.forEach(function (service) {
        if (self._serviceMap[service.fqdn]) { // add all addresses to existing service for cases when localhost is found as first
          createSet(self.services.filter(s => s.fqdn === service.fqdn)[0], service)
          return
        }
...
function createSet (currentService, newService) {
  let newAddresses = new Set(currentService.addresses.concat(newService.addresses))
  currentService.addresses = [...newAddresses]
}
